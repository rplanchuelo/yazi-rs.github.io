...
```
## Compress files with apack (atool)

Add the keybindings to your `keymap.toml`:

```toml
{ on = [ "<C-c>" ], exec = "plugin archiver --sync ", desc = "Compress files with apack" },
```

Create a folder $HOME/.config/yazi/plugins/archiver.yazi 

Create a file into this folder, init.lua
